...
// On force un prêtre pour pouvoir tester les talents
...
async function waitForMessage(room: Colyseus.Room, messageType: string, timeoutMs: number = 5000): Promise<any> {
    return new Promise((resolve, reject) => {
        ...
        const listener = (type: string | number, payload: any) => {
            if (!resolved && type === messageType) {
                resolved = true;
...
async function testTalentSystem(room: Colyseus.Room) {
    ...
    // --- ÉTAPE 1 : Stats initiales ---
    console.log("⏳ Demande des stats initiales au serveur...");
    const initialUpdate = await waitForMessage(room, "player_update");
    console.log("📊 Stats initiales:", initialUpdate);

    console.log("👉 Points de talent disponibles: " + initialUpdate.availableSkillPoints);

    // --- ÉTAPE 2 : Donner de l'XP --- 
    console.log("\n--- ÉTAPE 2 : Donner " + XP_AMOUNT_TO_LEVEL_UP + " XP ---");
    room.send("debug_give_xp", { amount: XP_AMOUNT_TO_LEVEL_UP });

    const levelUpUpdate = await waitForMessage(room, "player_update");
    console.log("✅ Mise à jour reçue:", levelUpUpdate);

    const statsAfterLevelUp = levelUpUpdate.stats;
    console.log("📊 Stats après level-up:", statsAfterLevelUp);

    console.log("👉 Points de talent disponibles: " + levelUpUpdate.availableSkillPoints);
    console.log("📊 DIFF →", diff(initialUpdate.stats, statsAfterLevelUp));
    ...
    const afterLearnUpdate = await waitForMessage(room, "player_update");
    console.log("📊 Stats après apprentissage du talent:", afterLearnUpdate);

    console.log("👉 Points de talent disponibles: " + afterLearnUpdate.availableSkillPoints);
    console.log("📊 DIFF →", diff(statsAfterLevelUp, afterLearnUpdate.stats));

    // --- ÉTAPE 4 : Reset des talents ---
    console.log("\n--- ÉTAPE 4 : Reset des talents ---");
    room.send("talent_reset");

    const afterResetUpdate = await waitForMessage(room, "player_update");
    console.log("📊 Stats après reset:", afterResetUpdate);

    console.log("👉 Points de talent disponibles: " + afterResetUpdate.availableSkillPoints);
    console.log("📊 DIFF →", diff(afterLearnUpdate, afterResetUpdate));
...
(async () => {
    try {
        ...
        if (!profile) {
            ...
            console.warn(
                `⚠️ Le personnage existant est un ${profile.class}. 
                Le test pourrait échouer si ce n'est pas un ${FORCED_CHARACTER_CLASS}.`
            );
        }
